...
public interface InmuebleRepository extends JpaRepository<Inmueble, Integer> {
        ...
        List<Object[]> contarInmueblesPorTipo();

        // Sumar el precio total de todos los inmuebles
        @Query("SELECT COALESCE(SUM(i.precio), 0) FROM Inmueble i")
        Double sumarPrecioTotal();
        ...
        @Query("SELECT i FROM Inmueble i " +
                        "LEFT JOIN i.interesados interesados " +
                        "GROUP BY i " +
                        "ORDER BY COUNT(interesados) DESC")
        List<Inmueble> findTop5ByInmuebles(Pageable pageable);
